...
$f = HTML::Form->parse(
  <<'EOT', base => "http://localhost/", verbose => 1 );
<form method=post>
...
ok( join( ":", map $_->name, $f->find_input( undef, "option" ) ),
  "x:y" );
...
ok( join( ":", $f->find_input( "r4" )->value_names ),
  "before and after" );
...
EOT
ok( join( ":", $f->find_input( "keep_informed" )->value_names ),
  "off:" );
