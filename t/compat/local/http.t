...
unless ( -f "t/compat/CAN_TALK_TO_OURSELF" ) {
  print
   "1..0 # Skipped: Can't talk to ourself (misconfigured system)\n";
  exit;
}
...
require
 IO::Socket;  # make sure this work before we try to make a HTTP::Daemon
...
if ( $D eq 'daemon' ) {
  ...
  open( DAEMON, "$perl t/compat/local/http.t daemon |" )
...
sub httpd_get_basic {
  ...
  if ( defined( $u ) && $u eq 'ok 12' && $p eq 'xyzzy' ) {
    ...
    $c->print(
      "WWW-Authenticate: Basic realm=\"libwww-perl\"\015\012" );
    $c->send_crlf;
  }
}
...
$ua->credentials( $req->url->host_port, "libwww-perl", "ok 12",
  "xyzzy" );
$res = $ua->request( $req );
ok( $res->is_success );

# Then illegal credentials
$ua->credentials( $req->url->host_port, "libwww-perl", "user",
  "passwd" );
$res = $ua->request( $req );
ok( $res->code, 401 );
