...
$m = HTTP::Message->new(
  [ content_type => "message/http; boundary=aaa", ], <<EOT);
GET / HTTP/1.1
...
my $NO_ENCODE = $] < 5.008
 || ( $Config{'extensions'} !~ /\bEncode\b/ ) ? "No Encode module" : "";
$@ = "";
skip(
  $NO_ENCODE,
  sub {
    eval { $m->decoded_content };
  },
...
my $BAD_ENCODE = $NO_ENCODE
 || !(
  eval { require Encode; defined( Encode::decode( "UTF-8", "\xff" ) ) }
 );

skip( $BAD_ENCODE, sub { $m->decoded_content }, "a\x{FFFD}" );
skip( $BAD_ENCODE, sub { $m->decoded_content( charset_strict => 1 ) },
  undef );
...
if ( $] >= 5.008001 ) {
  eval { HTTP::Message->new( [], "\x{263A}" ); };
  ok( $@ =~ /bytes/ );
  $m = HTTP::Message->new;
  eval { $m->add_content( "\x{263A}" ); };
  ok( $@ =~ /bytes/ );
  eval { $m->content( "\x{263A}" ); };
  ok( $@ =~ /bytes/ );
}
else {
...
$m = HTTP::Message->new( [ "Content-Type", "text/plain", ],
  "Hello world!" );
