...

{
  ...
  my %servers = (
    a => {
      "/" =>
       "HTTP/1.0 200 OK${CRLF}Content-Type: text/plain${CRLF}Content-Length: 6${CRLF}${CRLF}Hello\n",
      "/bad1" =>
       "HTTP/1.0 200 OK${LF}Server: foo${LF}HTTP/1.0 200 OK${LF}Content-type: text/foo${LF}${LF}abc\n",
      "/09" => "Hello${CRLF}World!${CRLF}",
      "/chunked" =>
       "HTTP/1.1 200 OK${CRLF}Transfer-Encoding: chunked${CRLF}${CRLF}0002; foo=3; bar${CRLF}He${CRLF}1${CRLF}l${CRLF}2${CRLF}lo${CRLF}0000${CRLF}Content-MD5: xxx${CRLF}${CRLF}",
      "/head" =>
       "HTTP/1.1 200 OK${CRLF}Content-Length: 16${CRLF}Content-Type: text/plain${CRLF}${CRLF}",
      "/colon-header" =>
  ...
  sub print {
    ...
    if ( $method eq "TRACE" ) {
      my $len = length( $in );
      $out
       = "HTTP/1.0 200 OK${CRLF}Content-Length: $len${CRLF}"
...
ok( $res->{code}, 200 );
ok( $res->{content},
  "TRACE /foo HTTP/1.1${CRLF}Keep-Alive: 300${CRLF}Connection: Keep-Alive${CRLF}Host: a${CRLF}${CRLF}"
);
...
ok( $res->{code}, "200" );
ok( $res->{content},
  "TRACE /foo HTTP/1.1${CRLF}Connection: close${CRLF}Host: a${CRLF}${CRLF}"
);
...
$h = HTTP->new( Host => "a", KeepAlive => 1, ReadChunkSize => 1 )
 || die;                         # reconnect
...
ok( "@{$res->{headers}}",
  "Content-Type text/plain Content-Length 6 Bad-Header :foo" );
