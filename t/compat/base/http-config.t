...
$conf->add_item(
  "slash",
  m_host_port   => "www.example.com:80",
  m_path_prefix => "/"
);
$conf->add_item(
  "u:p",
...
use HTTP::Request;
my $request
 = HTTP::Request->new( HEAD => "http://www.example.com/foo/bar" );
$request->header( "User-Agent" => "Moz/1.0" );

ok(
  j( $conf->matching_items( $request ) ),
  "u:p|slash|.com|GET|not secure|always"
);
...
ok( j( $conf->matching_items( "http://activestate.com" ) ),
  ".com|not secure|always" );
...
ok(
  j( $conf->matching_items( $response ) ),
  ".com|success|GET|secure|always"
);
